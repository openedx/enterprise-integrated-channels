...
class CornerstoneAPIClient(IntegratedChannelApiClient):
    ...
    def create_course_completion(self, user_id, payload):
        ...
        Raises:
            ClientError: if no matching Learning Assignments record exists yet for an in-progress
                update, so the transmission stays eligible for retry.
        """
    ...
    def _complete_transcript(self, data):
        ...
        url = urljoin(
            ...
            self.enterprise_configuration.transcript_complete_api_path,
        )
        transcript_payload = {
    ...
    def _update_learning_assignment(self, data):
        ...
        lookup_url = urljoin(
            self.enterprise_configuration.cornerstone_base_url,
            self.enterprise_configuration.learning_assignments_api_path,
        )
        start_time = time.time()
        ...
        try:
            ...
            LOGGER.info(
                generate_formatted_log(
                    ...
                    None,
                    data.get('courseId'),
                    'No learning assignment found for userGuid {user_guid} and learning object '
                    '{lo_id}.'.format(user_guid=data.get('userGuid'), lo_id=learning_object_id)
                )
            )
            raise ClientError(
                'CornerstoneAPIClient found no learning assignment for userGuid {user_guid} and '
                'learning object {lo_id}.'.format(
        ...
        update_url = urljoin(
            ...
            '{path}/{assignment_id}'.format(
                path=self.enterprise_configuration.learning_assignments_api_path, assignment_id=assignment_id
            ),
        )
        assignment_payload = {
    ...
    def get_oauth_url(self):
        ...
        return urljoin(
            self.enterprise_configuration.cornerstone_base_url,
            self.global_cornerstone_config.oauth_api_path,
        )
