...
class TestCornerstoneUtils(unittest.TestCase):
    ...
    def test_update_cornerstone_learner_data_transmission_audit(self, csod_params, course_id, expected_result):
        ...
        actual_result = request.user.cornerstone_ch_transmission_audit.filter(course_id=course_id).exists()
        assert actual_result == expected_result
        if expected_result:
            record = request.user.cornerstone_ch_transmission_audit.filter(course_id=course_id).first()
